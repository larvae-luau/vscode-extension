...
| `larvae.hideOutputFolder` | `true` | Hide larvae's output directory (`output` in `larvae.toml`) from the explorer via a managed `files.exclude` entry. Only touches folders containing a `larvae.toml`. |
